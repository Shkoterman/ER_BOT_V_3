...
from config import airt_api_key
import pickle
...
airt_calendar = airtable.Airtable(airt_app, airt_calendar_tbl, airt_api_key)
...
def for_cancel_reg_event_list(username): # вернёт дикт с айди записи и названием мероприятия
    ...
    for i in range(len(future_events_resp)):
        ev_name = future_events_resp[i]['fields']['Name_event'].strip()
        ev_id = future_events_resp[i]['id']
...
def add_reg(for_reg_dickt): # вносит запись в регистрацию
    airt_reg.insert(
        {airt_reg_tbl_event_for_reg_field: for_reg_dickt['event_id'].split(' '),
         ...
         airt_reg_tbl_subscriber_field: for_reg_dickt['user_is_sub']
        })
        ...
        for i in range(len(future_events_resp)):
            ...
            if future_events_resp[i]['id']==event_id:
                ...
                if desc_places<1:
                    ...
                    desc_places='*Свободных мест: *' + str(desc_places) + '\n'

                try:
                    desc_text = future_events_resp[i]['fields']['Описание'].strip().replace('\\', '')
                    if desc_text!='':
                        replace_dictionary = ['_', '*', '`']
                        for key in replace_dictionary:
                            desc_text = desc_text.replace(key, "\\" + key)
                        desc_text = '*Описание:*\n' + desc_text + '\n'
                except:
                    desc_text = ''
...
def get_calendar_section():
    event_list=airt_calendar.get_all(view=airt_calendar_view)
