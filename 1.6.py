...
def Start(m):                                                               #первая встреча с дорогим пользователем
    inlogtxt='@' + m.from_user.username + ' нажал \start'
    write_in_log(inlogtxt)                                                  #писька в лог
    ...
    if m.from_user.username in adminlist:                                   #если чел в админ листе добавляю админские кнопки
        ...
        write_in_log(inlogtxt='@' + m.from_user.username + ' взял(а) админский доступ')                                              #писька в лог
...
def handle_text(message):
    ...
    if message.text.strip() == 'Мои регистрации':                           #тут и дальше тект сообщения интерпритируется как команда
        try:
            ...
            main_menu(message)
        write_in_log(inlogtxt='@'+message.from_user.username +' запросил свои регистрции')              #писька в лог
    ...
    # rassylka
        ...
        write_in_log(inlogtxt= '@' + message.from_user.username + ' sent:\n' + message.text+ '\n')
        main_menu(message)

def write_in_log(inlogtxt):                                                 #запись текста в лог
    inlogtxt = datetime.now().strftime("%d-%m-%Y %H:%M") + ': ' + inlogtxt  #дописываю время
...
def add_user(m):                                                            # add to dict if there is no {nick: chatid <-/-> chatid: nick}
    nick = '@' + m.from_user.username.lower()                               #добавляю @ и делаю всё строчными
    if nick==None:                                                          #если ника нет или скрыт, делаю искусственный ник
    ...
    if nick not in user_names_chatid_dict or user_id not in user_names_chatid_dict: #если в локальной бд нет ника или айди добавляю
        ...
        write_in_log(inlogtxt=nick + 'добавился в юзерлист')
    with open('user_names_chatid.pkl', 'wb') as f:
...
def send_spam(message, message_for_spam, list_of_spam, list_of_spam_niks):  #сопсно рассылка
    ...
    if message.text ==  'Разослать':
        ...
        write_in_log(inlogtxt=message.from_user.username + ' дал рассылку')
        main_menu(message)
...
def chose_feedack_event(message):
    ...
    for i in range(len(response_reg)):
        ...
        if evenday == date.today() - timedelta(days=1):
            name_event.append(eventname.strip())
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
def give_feedback(message, name_event):
    print(name_event)
    if message.text in name_event or ' ' + message.text in name_event:
