...
def clear_handels_for_all():
    print('wait untill clearing handlers')
    all_user_id=ER_DB.get_all_id()
    markup = btns.user_main_menu_markup
    adminmarkup=btns.admin_main_menu_markup
    mymarkup=btns.my_main_menu_markup
    admin_list = ER_DB.get_admin_list()
    admin_id_list = ER_DB.find_user_id_or_nick(admin_list)
    for chat_id in all_user_id:
        try:
            if chat_id==shkoterman_chat_id:
                bot.clear_step_handler_by_chat_id(chat_id)
                bot.send_message(chat_id, text=strs.main_menu_text, reply_markup=mymarkup, disable_notification=True)
            elif chat_id in admin_id_list:
                bot.clear_step_handler_by_chat_id(chat_id)
                bot.send_message(chat_id, text=strs.main_menu_text, reply_markup=adminmarkup, disable_notification=True)
            else:
                bot.clear_step_handler_by_chat_id(chat_id)
                bot.send_message(chat_id, text=strs.main_menu_text, reply_markup=markup, disable_notification=True)
        except:
            pass
