...
def query_handler(call):
    ...
    if what_did_you_like_list[call.from_user.id][int(call.data)][0]=='➖':
        what_did_you_like_list[call.from_user.id][int(call.data)]='✔ '+what_did_you_like_list[call.from_user.id][int(call.data)][1:]
...
def registration_step_2(message, for_reg_dickt, open_for_reg_events):
    if message.text in open_for_reg_events.keys():
        if '@'+message.from_user.username.lower() in ER_DB.tgnicks_of_event_R_and_WL(open_for_reg_events[message.text][0]):
            ...
            if message.text in open_for_reg_events.keys():
                ...
                if for_reg_dickt['ev_is_sub']==True and for_reg_dickt['user_is_sub']==False:
                    ...
                    markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add(btns.minusonebtn, btns.backbtn, btns.plusonebtn)
