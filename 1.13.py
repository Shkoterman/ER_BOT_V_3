    ...
    if message.text == 'Да':
        ...
        if user_nick in user_event_names_dict.keys():
            if reg_event_name in user_event_names_dict[user_nick]:
                bot.send_message(message.chat.id, text=user_nick + ' уже зарегистрирован_а на мероприятие '+reg_event_name.split(',')[0])
                main_menu(message)
        ...
        if message.text == 'Главное меню':
            ...
            if new_user_nick in user_event_names_dict.keys():
                if reg_event_name in user_event_names_dict[new_user_nick]:
                    bot.send_message(message.chat.id,
                                     text=new_user_nick + ' уже зарегистрирован_а на мероприятие ' + reg_event_name.split(':')[0])
...
def send_for_reg(message, reg_event_ID, reg_event_name, user_nick, user_name):
    ...
    if reg_event_name not in get_registration_list.avalible_event_name_event_id_dict_full.keys():
        ...
        reg_event_name=reg_event_name[:-7]+str(int(reg_event_name[-7:-5])-1)+reg_event_name[-5:]
        print(reg_event_name)
        print(user_event_names_dict[user_nick])
        print(reg_event_name==user_event_names_dict[user_nick])
        if user_nick[0]!='@':
            user_nick='@'+user_nick
        if reg_event_name in user_event_names_dict[user_nick]:
            bot.send_message(message.chat.id,
                             text=user_nick+' зарегистрирован_а на ' + reg_event_name.split(':')[0])
            bot.send_message(message.chat.id,
