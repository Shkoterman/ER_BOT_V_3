...
def registration_step_2(message, for_reg_dickt, open_for_reg_events):
    ...
    if message.text in open_for_reg_events.keys():
        ...
        if '@' + message.from_user.username.lower() in ER_DB.tgnicks_of_event_R_and_WL(
            ...
            for_reg_dickt['event_name'] = message.text
...
def event_calendar_1(message):
    ...
    calendar_tags=['all']    #лист тэгов movie
...
def event_calendar_2(message, resp, calendar_sectoin_events, calendar_tags):
    ...
    if message.text==btns.mainmenubtn.text:
        ...
        for i in range(len(resp[1])):
            try:
                if event_teg in resp[1][i]['fields']['tag'] or event_teg == 'all':
        ...
        if len(section_time_gate_list)==1 or len(section_event_list)==1:
            event_calendar_4(message, section_event_list, calendar_sectoin_events, calendar_tags, True)
            ...
            bot.register_next_step_handler(send, event_calendar_3, section_event_list, time_tag_name_dict)
...
def event_calendar_3(message, section_event_list, time_tag_name_dict):
    ...
    if message.text == btns.mainmenubtn.text:
        ...
        event_calendar_4(message, section_event_list, section_event_list, time_tag_name_dict)
        ...
        event_calendar_4(message, event_list, section_event_list, time_tag_name_dict)
    else:
        bot.send_message(message.chat.id, text=strs.didnt_get_it, reply_markup=btns.skip_all_handlers)
        bot.register_next_step_handler(message, event_calendar_3, section_event_list, time_tag_name_dict)
def event_calendar_4(message, event_list, section_event_list, time_tag_name_dict, back_leeds_to_sections=False):
    ...
    for i in range(len(event_list)):
        ...
        try: description = '*' + strs.description + '*' + event_list[i]['fields']['event_discriptoin']+'\n'
        except: description=''
        try:
        ...
        try:
            tags_raw = '#'+' #'.join(event_list[i]['fields']['tag'])
            tags = '_' + strs.tags + tags_raw + '_'
        except:
            tags = ''
        message_text = name+description+begining+ending+cost+link+tags
        send = bot.send_message(message.chat.id, text=message_text, disable_web_page_preview=True, parse_mode='Markdown')
