...
def main_menu(message, frsttime):
    ...
    if frsttime == 1:
        ...
        write_in_log(message, 'зашёл как с frsttime == 1')
    elif frsttime == 2:
        text = strs.time_out_text
        bot.clear_step_handler_by_chat_id(message.chat.id)
        write_in_log(message, 'выкинуло по таймауту')
    elif frsttime==3:
        ...
        text = strs.main_menu_text
        time_out[message.chat.id] = 0
        write_in_log(message, 'нажал_а отмену или получил отказ')

    bot.send_message(message.chat.id, text=text, reply_markup=markup)


def check_registration(message):
    write_in_log(message, 'проверил регистрации')
...
def registration_step_1(message):
    write_in_log(message, 'начал регистрацию')
    if message.from_user.username == None:
        bot.send_message(message.chat.id, text=strs.need_dog_msg)
        main_menu(message, 0)
    else:
...
def registration_step_2(message, for_reg_dickt, open_for_reg_events):
    if message.text in open_for_reg_events.keys():
        write_in_log(message, 'пытался зарегестрироваться на '+message.text)
...
def registration_step_5(message, for_reg_dickt):
    ...
    write_in_log(message, 'отправил регистрацию: '+str(for_reg_dickt))
    main_menu(message, 3)
...
def feedback_step_1(message):
    write_in_log(message, 'нажал дать обратную связь')
...
def feedback_step_6(message, feedback_dickt):
    if message.text == btns.backbtn.text:
        ...
        try:
            ...
            pass
        write_in_log(message, 'дал обратную связь: '+str(feedback_dickt))
...
def cancel_reg_step_1(message):
    write_in_log(message, 'нажал отменить регистрацию')
    if message.from_user.username == None:
        bot.send_message(message.chat.id, text=strs.need_dog_msg)
    else:
