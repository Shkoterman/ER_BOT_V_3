            ...
            if future_events_resp[i]['id']==event_id:
                try:
                    desc_time_row = datetime.strptime(future_events_resp[i]['fields']['Начало Мероприятия'],
                                                  '%Y-%m-%dT%H:%M:%S.%fZ') + timedelta(hours=1)
                ...
                try:
                    ...
                    if type(desc_all_price)==int or type(desc_all_price)==int:
                        desc_price='*Стоимость для* [more](https://t.me/ensaladaru/1319)*: *' \
                               + str(future_events_resp[i]['fields']['Стоимость д. клуба']) + '\n' \
                               '*Стоимость для всех: *' + str(future_events_resp[i]['fields']['Стоимость д. всех'])
                except: desc_price=''
